...
//     fprintf(stderr, "%d x %d\n", w, h);
    ...
    // download
    {
        ...
        if (m_zcExportBuf != VK_NULL_HANDLE)
        {
            // === Phase-1b zero-copy ===
            // Keep out_gpu on the GPU. Finish the net, then raw vkCmdCopyBuffer the
            // result (interleaved RGB8 in the int8 path) into the caller's
            // OPAQUE_FD-exportable buffer, on ncnn's own VkDevice/queue, BEFORE the
            // blob allocator is reclaimed. No CPU download of the output.
            cmd.submit_and_wait();

            const size_t want = (size_t)out_gpu.w * (size_t)out_gpu.h * 3u;
            const size_t copyBytes = want < m_zcExportCap ? want : m_zcExportCap;

            VkDevice dev = vkdev->vkdevice();
            const uint32_t qfi = vkdev->info.compute_queue_family_index();

            VkCommandPoolCreateInfo zpci = {};
            zpci.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
            zpci.queueFamilyIndex = qfi;
            VkCommandPool zpool = VK_NULL_HANDLE;
            vkCreateCommandPool(dev, &zpci, 0, &zpool);

            VkCommandBufferAllocateInfo zabi = {};
            zabi.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
            zabi.commandPool = zpool;
            zabi.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
            zabi.commandBufferCount = 1;
            VkCommandBuffer zcb = VK_NULL_HANDLE;
            vkAllocateCommandBuffers(dev, &zabi, &zcb);

            VkCommandBufferBeginInfo zbi = {};
            zbi.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
            zbi.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
            vkBeginCommandBuffer(zcb, &zbi);

            VkBufferCopy zregion = {};
            zregion.srcOffset = out_gpu.buffer_offset();
            zregion.dstOffset = 0;
            zregion.size = copyBytes;
            vkCmdCopyBuffer(zcb, out_gpu.buffer(), m_zcExportBuf, 1, &zregion);
            vkEndCommandBuffer(zcb);

            VkQueue zq = vkdev->acquire_queue(qfi);
            VkSubmitInfo zsi = {};
            zsi.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
            zsi.commandBufferCount = 1;
            zsi.pCommandBuffers = &zcb;
            vkQueueSubmit(zq, 1, &zsi, VK_NULL_HANDLE);
            vkQueueWaitIdle(zq);
            vkdev->reclaim_queue(qfi, zq);

            vkDestroyCommandPool(dev, zpool, 0); // also frees zcb
...
#ifdef ARBIT_RIFE_ZEROCOPY
int RIFE::process_v4_to_buffer(const ncnn::Mat& in0image, const ncnn::Mat& in1image,
                               float timestep, VkBuffer exportBuf, size_t exportCapacity,
                               int& outW, int& outH) const
{
    m_zcExportBuf = exportBuf;
    m_zcExportCap = exportCapacity;
    m_zcOutW = 0;
    m_zcOutH = 0;

    ncnn::Mat dummy; // unused in the zero-copy branch
    const int ret = process_v4(in0image, in1image, timestep, dummy);

    m_zcExportBuf = VK_NULL_HANDLE;
    m_zcExportCap = 0;
    outW = m_zcOutW;
    outH = m_zcOutH;
    return ret;
